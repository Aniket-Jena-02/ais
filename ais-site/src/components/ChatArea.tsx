...
import MessageInput, { type FileAttachment } from "./MessageInput/MessageInput"
...
import AddMemberModal from "./forms-and-dialogs/AddMemberModal"
import MembersPanel from "./right-side/MembersPanel"
...
const ChatArea = () => {
  ...
  useEffect(() => {
    ...
    socket.on("message_reaction", (data: { messageId: string; reactions: Reaction[] }) => {
      ...
      // scroll to bottom only when reacted on the last message
      if (data.messageId === latestMessageId) {
        scrollToBottom("smooth")
      }
    })
